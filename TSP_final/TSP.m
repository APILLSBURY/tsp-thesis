function [sp_labels] = TSP(K, root, files, dispOn, frames)
    ...
    for f=frames
        ...
        if (frame_it>1)
            ...
            if frame_it==2
                save('pre_resize.mat');
                [IMG_K, IMG_label, IMG_SP, ~, IMG_max_UID, ~, IMG_SP_old] = move_resize(IMG_label, IMG_w, flow, IMG_K, IMG_SP, IMG_new_pos, IMG_new_app, IMG_max_UID, IMG_max_SPs, IMG_data, IMG_boundary_mask, model_order_params, IMG_alive_dead_changed, IMG_N, IMG_new_SP, IMG_SP_old, IMG_SP_changed, 8);
            end
        ...
        while (~converged && it<20)
            ...
            if (~IMG_params.reestimateFlow)
                save('pre_localonly.mat', 'IMG_label');
save('pre_localonly_all.mat');
