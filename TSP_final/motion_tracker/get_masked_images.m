...
function [ims] = get_masked_images(frame_inc, root, flow_folder, expansion, percentage_tolerance, min_object_percentage, cutoff_iters, test, sameness_threshold, std_threshold, min_frames_per_object)
    disp('Getting masked images...');
    [objects] = find_masks([root flow_folder], expansion, percentage_tolerance, min_object_percentage, cutoff_iters, sameness_threshold, std_threshold, min_frames_per_object);
    fprintf('%d objects found\n', numel(objects));
    if test
        combined_final_masks = masks;
    else
        if numel(objects) < 1 || size(objects(1).masks, 3) < 2
            error('not enough masks');
        end
        combined_final_masks = false(size(objects(1).masks,1), size(objects(1).masks,2), (size(objects(1).masks,3)+1)*frame_inc+1, numel(objects));
        %combined_final_masks = false(size(objects(1).masks,1), size(objects(1).masks,2), (size(objects(1).masks,3)+1)*frame_inc+1);
        for o=1:numel(objects)
            final_masks = false(size(combined_final_masks, 1), size(combined_final_masks, 2), size(combined_final_masks, 3));
            %final_masks = false(size(combined_final_masks));
            ...
            combined_final_masks(:,:,:,o) = final_masks;
            %combined_final_masks = combined_final_masks | final_masks;
        end
    end
    for o=1:numel(objects)
        ims = mask_images(combined_final_masks(:,:,:,o), root, frame_inc);
        write_video(ims, [num2str(o) 'eli.avi'], 10);
    end
    real_final_masks = false(size(combined_final_masks(:,:,:,1)));
    for o=1:numel(objects)
        real_final_masks = real_final_masks | combined_final_masks(:,:,:,o);
    end
    ims = mask_images(real_final_masks, root, frame_inc);
end
...
function objects = find_masks(root, expansion, percentage_tolerance, min_object_percentage, cutoff_iters, sameness_threshold, std_threshold, min_frames_per_object)
    ...
    for f=1:max_f
        ...
        for l=1:size(object_masks, 3)
            ...
            if mean([col_std(col_std>0) row_std(row_std>0)']) > std_threshold
                % if it is, only use the connected portion
%                 split_expanded_mask = find_levels(expanded_mask);
%                 max_size = 0;
%                 for ll=1:size(split_expanded_mask, 3)
%                     if sum(sum(split_expanded_mask(:,:,ll))) > max_size
%                         max_size = sum(sum(split_expanded_mask(:,:,ll)));
%                         expanded_mask = split_expanded_mask(:,:,ll);
%                     end
%                 end
    ...
    % post-processing on objects
    save('hanover1.mat', 'objects');
