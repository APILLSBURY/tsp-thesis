...
function [masks, theta_medians] = find_masks(root, expansion, percentage_tolerance, min_object_percentage, cutoff_iters)
    ...
    files = dir([root '*.mat']);
    masks = [];
    theta_medians = zeros(numel(files), 1);
    
    max_f = numel(files);
    
    found_object = false; % boolean to determine when we first find the object
    ...
    for f=1:max_f
        ...
        for l=1:size(object_masks, 3)
            ...
            fprintf('%f\n', mean([col_std(col_std>0) row_std(row_std>0)']));
            ...
            if mean([col_std(col_std>0) row_std(row_std>0)']) > 0.1 % check to see if it's an object
         ...
         if f>1 && max_sameness<0.1 && sum(sum(masks(:,:,f-1)))>0
             % if there's nothing like the previous frame, assume that the
         ...
         if found_object
             for ff=1:(f-2)
                 masks(:,:,ff) = masks(:,:,f-1);
                 theta_medians(ff) = theta_medians(f-1);
             end
         end
    end
end
